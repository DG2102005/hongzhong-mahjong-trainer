  ...
  return (
    <div className="hand-wrap">
      <div className="hand-row">
        {displayHand.map((tl, i) => {
          const isDrawn = drawnTileId != null && tl.id === drawnTileId;
          const hasGapBefore = showGaps && i > 0 && effectiveGaps[i - 1];
          const isDragging = dragId === tl.id;
          const showDropBefore = dropIdx === i && dragId != null && dragId !== tl.id;
          return (
            <span
              key={tl.id}
              className={`tile-slot${hasGapBefore ? ' gap-before' : ''}${showDropBefore ? ' drop-before' : ''}${isDragging ? ' dragging' : ''}`}
              draggable={mode === 'decompose'}
              onDragStart={(e) => handleDragStart(e, tl.id)}
            ...
            >
              <Tile
                tile={tl}
                size={46}
                selected={selected === tl.id}
                onClick={() => handleClick(tl)}
                highlight={isDrawn}
              />
              {isDrawn && <span className="drawn-mark" title={t('hand.new')}>{t('hand.new')}</span>}
            </span>
          );
        })}
      </div>
      <div className="hand-toolbar">
        <span className="hand-hint">
          {mode === 'decompose'
            ? t('hand.hint.decompose')
            : selected !== null ? t('hand.hint.confirm') : t('hand.hint.play')}
        </span>
        <button
