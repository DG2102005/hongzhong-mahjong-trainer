  ...
  return (
    <div className={`player-seat seat-${position} ${isTurn ? 'seat-active' : ''}`}>
      <div className="seat-header">
        <span className="seat-name">{seatName(player.seat)} · {player.name}</span>
        {player.isDealer && <span className="dealer-mark">{t('seat.dealer')}</span>}
        {isTurn && <span className="turn-indicator">{t('seat.thinking')}</span>}
        {isGameOver && isWinner && <span className="winner-tag">{t('seat.won')}</span>}
        {isGameOver && state.isDraw && <span className="draw-tag">{t('seat.draw')}</span>}
      </div>
      ...
      <div className="seat-stats">{t('seat.stats', { n: handCount, m: meldCount })}</div>

      {/* 局终显示弃牌明细 */}
      {isGameOver && player.discards.length > 0 && (
        <div className="seat-discards-end">
          {t('seat.discards')}{player.discards.map((t) => tileLabel(t)).join(' ')}
        </div>
      )}
    </div>
  );
