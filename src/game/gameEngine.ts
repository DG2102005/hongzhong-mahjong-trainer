...
export function drawTile(state: GameState, seat: Seat): GameState {
  ...
  return completeDraw(s, seat, tile);
}

// 自由进牌(复盘演绎用): 从牌墙剩余牌中任选一张摸入
export function drawSpecificTile(state: GameState, seat: Seat, tileId: number): GameState {
  const s = clone(state);
  const idx = s.deck.findIndex((t) => t.id === tileId);
  if (idx < 0) return s;
  const [tile] = s.deck.splice(idx, 1);
  return completeDraw(s, seat, tile);
}

// 摸入某张牌后的公用收尾(胡判定/自摸操作/出牌阶段)
function completeDraw(s: GameState, seat: Seat, tile: Tile): GameState {
